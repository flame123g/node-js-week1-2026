...
async function readMembers(filePath) {
  ...
  try{
    ...
    console.log(`[錯誤]讀取檔案失敗: ${err.message}`);
  };
};
...
function filterVIP(members) {
  ...
  const filterMember=members.filter(member=>member.level==='VIP');
  return filterMember;
}
...
function sumCredits(members) {
  // TODO: 實作此函式
  // 提示：用 reduce，初始值給 0
  const sumMemberCredits=members.reduce((acc,currentValue)=>{
    return acc+currentValue.credits;
  },0);
  return sumMemberCredits || 0;
}
...
function getGymConfig() {
  // TODO: 實作此函式
  // 提示：用 || 給預設值
  const gymInfor={gymName: process.env.GYM_NAME || "未命名健身房", adminName: process.env.ADMIN_NAME || "尚未指派", defaultMembersPath: process.env.DEFAULT_MEMBERS_PATH || undefined};
  return gymInfor;
...
async function getVIPSummary(filePath) {
  ...
  const data =await readMembers(filePath);
  const filterMember=filterVIP(data);
  const mapName=filterMember.map(member=>member.name);
  const calculateCredits=sumCredits(filterMember);
  return {count:filterMember.length , totalCredits: calculateCredits, names: mapName};
}
